...
{
    [Header("FX")]
    [SerializeField] private ArrivalFxView2 systemJumpFlash;
    ...
    private void Start()
    {
        ...
        if (_eventBus == null)
        {
            Debug.LogError("[SystemTransitionController2] SimpleEventBus not found.");
            return;
        }

        if (_systemTravelService == null)
        {
            Debug.LogError("[SystemTransitionController2] ISystemTravelService not found.");
            return;
        }

        if (_gameSessionService == null)
        {
            Debug.LogError("[SystemTransitionController2] IGameSessionService not found.");
            return;
        }

        _eventBus.Subscribe<SystemTravelCompletedEvent>(OnTravelCompleted);
        LogCustom("SystemTravelCompletedEvent subscribed");
    }
    ...
    private void OnTravelCompleted(SystemTravelCompletedEvent evt)
    {
        ...
        {
            if (IsDebug())
                Debug.Log($"SystemTransitionController2: '{evt.TargetSystemId}' is the current system.");
        ...
        if (IsDebug())
            Debug.Log($"SystemTransitionController2: travel result = {result}");                
        ...
        // Debug.LogError("[SystemTransitionController2] evt = " + evt);
        ...
        // {
        //     Debug.LogError("[SystemTransitionController2] Cannot transition: SystemLink is null.");
        //     return;
        // }

        // Debug.LogError("[SystemTransitionController2] Transit to system = " + evt.SystemLink.LinkedSystem.Id);
